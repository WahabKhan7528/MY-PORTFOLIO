...
export default function Hero() {
  ...
  return (
    <section
      ref={container}
      className="relative w-full min-h-screen flex items-center justify-center overflow-hidden pt-28 sm:pt-32 md:pt-0"
    >
      <div
        ...
        >
          <h1 className="hero-heading text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-display font-bold leading-tight mb-6">
            <span className="inline-block overflow-visible">
              {"MERN Stack Developer".split(" ").map((word, i) => (
                <span
                  key={i}
                  className="hero-heading-word inline-block mr-3 lg:mr-4"
                >
                  {word}
                </span>
              ))}
          ...
          <p className="hero-desc text-lg sm:text-xl md:text-2xl text-gray-400 font-light mb-12 max-w-2xl">
            Crafting immersive, full-stack web applications with React, Node.js,
            Express, and MongoDB. Transforming complex ideas into seamless,
            interactive reality.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <a
              href="#projects"
              className="hero-cta px-8 py-4 rounded-full bg-white text-black font-medium text-sm tracking-wide transition-transform hover:scale-105 active:scale-95"
            >
              View My Work
            </a>
            <a
              href="#contact"
              className="hero-cta px-8 py-4 rounded-full border border-white/30 text-white font-medium text-sm tracking-wide bg-white/5 backdrop-blur-md transition-transform hover:scale-105 active:scale-95 hover:bg-white/10"
            >
              Get In Touch
            </a>
          </div>
