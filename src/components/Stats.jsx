    ...
    <section ref={container} className="stats-container flex items-center bg-black border-y border-white/10 relative overflow-hidden py-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 w-full">
        <div className="flex items-center gap-4 mb-8">
          ...
          <span className="text-sm font-mono tracking-[0.6em] text-white/70 uppercase">Core Metrics</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 border border-white/10">
          {stats.map((stat, index) => (
            <div
              key={index}
              className={`stat-box group p-8 sm:p-10 md:p-14 flex flex-col items-start justify-between relative transition-all duration-500 hover:bg-white/[0.04] overflow-hidden ${index % 4 !== 3 ? 'md:border-r border-white/10' : ''
            ...
            >
              ...
              {/* ID */}
              <div className="mb-10">
                <span className="text-sm font-mono text-white/40 tracking-widest group-hover:text-white/80 transition-colors duration-300">
                  [{stat.id}]
                </span>
              </div>
              ...
              <div className="mt-10 w-full flex justify-between items-end">
                <div className="text-xs sm:text-sm md:text-base text-white/50 font-mono tracking-[0.4em] uppercase group-hover:text-white transition-all duration-300">
                  {stat.label}
                </div>
                <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-500 text-[10px] sm:text-[11px] md:text-[12px] font-mono text-white/40 tracking-tighter animate-pulse">
                  // VERIFIED
                </div>
              </div>
