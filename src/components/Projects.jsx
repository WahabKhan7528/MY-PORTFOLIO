...
const projects = [
  ...
  },
  {
    ...
    category: "WEB-APP",
    year: "2026",
    description: "Secure client-side encryption directly in the browser.",
    demoUrl: "#",
...
export default function Projects({ onProjectSelect }) {
  ...
  return (
    <section
      id="projects"
      ref={containerRef}
      className="relative w-full h-screen overflow-hidden bg-black text-white z-[60]"
    >
      <div
        ref={horizontalRef}
        className="flex h-full w-fit relative z-10"
      >
        {projects.map((project, index) => {
          return (
            <div
              key={project.id}
              className="project-slide w-screen h-full relative flex items-center justify-center overflow-hidden"
            >
              ...
              <div className="absolute inset-0 z-0">
                {project.image ? (
                  <img
                    ...
                    loading="lazy"
                    decoding="async"
                    className="w-full h-full object-cover brightness-[0.9] transition-transform duration-[2s] hover:scale-105"
                  />
                ) : (
              ...
              <div className="absolute top-24 right-8 md:top-36 md:right-12 z-20 flex flex-col items-center justify-center min-w-[80px] h-[80px] md:min-w-[100px] md:h-[100px] bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 shadow-2xl">
                <span className="text-[32px] md:text-[44px] font-display font-black leading-none text-white select-none tracking-tighter">
                  0{index + 1}
                </span>
              ...
              <div className="absolute bottom-16 left-8 md:bottom-24 md:left-20 z-20">
                <div className="project-info-inner">
                  <button
                    onClick={() => onProjectSelect(project)}
                    className="group flex items-center gap-4 px-8 py-4 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-xs tracking-[0.2em] uppercase font-bold text-white hover:bg-white hover:text-black transition-all duration-500 shadow-2xl"
                  >
                    Explore Work
