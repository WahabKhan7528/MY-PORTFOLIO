...
const socialLinks = [
  {
    ...
    icon: (
      <svg className="w-5 h-5" viewBox="0 0 24 24">
        <path className="fill-current transition-colors duration-300 group-hover:fill-[#0A66C2]" d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
      </svg>
    ),
  },
  {
...
export default function ContactSection({ isHero = false }) {
  ...
  useGSAP(
    () => {
      ...
      const meta = q(".contact-meta");
      const title = q(".contact-title");
      const infoItems = q(".info-item");
      const form = q(".form-container");
      const glow = q(".glass-glow");
  ...
  return (
    <section
      id="contact"
      ref={container}
      className={`relative w-full bg-black overflow-hidden flex flex-col items-center justify-center px-4 sm:px-6 md:px-12 lg:px-24 ${isHero ? 'min-h-[70vh] py-20' : 'py-24 sm:py-32 md:py-48'}`}
    >
      ...
      <div className="relative z-10 w-full max-w-7xl">
        <div className="grid lg:grid-cols-2 gap-12 md:gap-16 lg:gap-32">

          {/* Left Column: Direct Info */}
          <div className="flex flex-col justify-center max-w-full lg:max-w-xl">
            <h2 className="contact-title text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-display font-black text-white tracking-tighter leading-[0.85] mb-12">
              Let's<br />
            ...
            <div className="space-y-12 mb-16">
              {PERSONAL.contactMethods.map((method, idx) => (
                <div key={idx} className="info-item group">
                  ...
                  <a
                    href={method.href}
                    className="text-xl sm:text-2xl md:text-3xl font-light text-white/90 hover:text-white transition-all inline-flex items-center gap-3 group-hover:translate-x-2 duration-300 min-h-[44px]"
                  >
                    {method.value}
              ...
              <div className="flex flex-wrap gap-4">
                {socialLinks.map((social, idx) => (
                  <a
                    ...
                    className="group relative p-5 min-h-[44px] border border-white/10 flex flex-col justify-between gap-6 overflow-hidden transition-all duration-500 hover:border-white/30 hover:bg-white/[0.02] rounded-none w-full sm:w-[180px]"
                    title={social.name}
                  >
                    {/* Corner Accents */}
          ...
          <div
            className="form-container relative w-full p-6 sm:p-10 md:p-12 border border-white/5 transition-all duration-700 overflow-hidden group/form hover:border-white/20 hover:bg-white/[0.04] hover:backdrop-blur-xl"
            ref={formRef}
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-10 sm:space-y-14 relative z-10">
              ...
              <div className="group relative pt-8">
                <label className="absolute -top-2 left-0 text-xs sm:text-sm font-mono text-white/60 uppercase tracking-[0.3em] transition-all duration-500 group-focus-within:text-white group-focus-within:-translate-y-1">Subject</label>
                <input
                  type="text"
                  name="subject"
              ...
              <div className="group relative pt-8">
                <label className="absolute -top-2 left-0 text-xs sm:text-sm font-mono text-white/60 uppercase tracking-[0.3em] transition-all duration-500 group-focus-within:text-white group-focus-within:-translate-y-1">Message Payload</label>
                <textarea
                  name="message"
              ...
              <button
                ...
                className="group relative w-full py-7 min-h-[44px] bg-transparent border border-white/20 text-white font-bold uppercase tracking-[0.5em] overflow-hidden transition-all duration-700 hover:border-white hover:text-black rounded-none"
              >
