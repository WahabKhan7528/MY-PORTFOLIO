...
export default function ProjectDetail({ project, onClose }) {
  ...
  return (
    <div
      ref={overlayRef}
      className="fixed inset-0 z-[9999] bg-black text-white overflow-y-auto no-scrollbar ios-scroll"
    ...
    >
      ...
      <div className="fixed top-0 left-0 w-full p-3 sm:p-4 md:p-6 lg:p-10 flex justify-between items-center z-[10000] border-b border-white/10 bg-black/80 backdrop-blur-md">
        <div className="flex items-center gap-3 md:gap-6">
          <div className="flex flex-col">
            <span className="text-[10px] md:text-xs font-mono tracking-[0.4em] text-white/70 uppercase leading-none mb-1">PROJECT_ID</span>
            <span className="text-sm md:text-base font-mono font-bold tracking-widest uppercase">NODE_{project.id || "00"}</span>
          </div>
        </div>

        <button
          onClick={handleClose}
          className="group flex items-center gap-2 md:gap-4 px-4 md:px-8 py-2 md:py-3 min-h-[44px] bg-white text-black text-[10px] md:text-xs font-bold tracking-[0.2em] md:tracking-[0.4em] uppercase hover:bg-gray-200 transition-all duration-300"
        >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 lg:px-12 pt-24 sm:pt-28 md:pt-32 lg:pt-40 pb-16 sm:pb-20 md:pb-24 relative z-10">
        <div className="grid lg:grid-cols-12 gap-12 md:gap-24">
          ...
          <div className="lg:col-span-8 space-y-16 md:space-y-24">
            <header className="detail-header space-y-4 sm:space-y-6 md:space-y-8">
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <span className="text-[10px] sm:text-xs md:text-sm font-mono tracking-[0.6em] md:tracking-[0.8em] text-white/70 uppercase">CASE_STUDY</span>
              </div>
              <h1 className="text-[clamp(2.5rem,8vw,8rem)] font-display font-black tracking-tighter leading-[0.9] break-words uppercase">
                {project.title.split(/[_\s]+/)[0]}<br />
                <span className="text-white/20">{project.title.split(/[_\s]+/).slice(1).join(' ') || "SYSTEM"}</span>
              </h1>
            ...
            <div className="space-y-8 sm:space-y-10 md:space-y-12">
              <div className="detail-visual-main relative group overflow-hidden border border-white/10 p-2 sm:p-3 md:p-4 bg-white/5 backdrop-blur-sm w-full h-auto">
                <img
                  src={activeImage}
                  alt={project.title}
                  className="w-full aspect-video object-cover transition-all duration-1000 shadow-2xl"
                />
                ...
                <div className="absolute top-2 sm:top-4 md:top-6 lg:top-8 left-2 sm:left-4 md:left-6 lg:left-8 flex items-center gap-2 sm:gap-3">
                  ...
                  <span className="text-[9px] sm:text-[10px] md:text-xs lg:text-sm font-mono tracking-[0.2em] md:tracking-[0.3em] text-white/50 uppercase">ACTIVE_FEED</span>
                </div>
              ...
              <div className="space-y-4 sm:space-y-5 md:space-y-6 overflow-hidden">
                <div className="flex justify-between items-end px-1 sm:px-2">
                  <h4 className="text-[10px] sm:text-xs md:text-sm font-mono tracking-[0.4em] md:tracking-[0.5em] text-white/70 uppercase">GALLERY</h4>
                  <span className="text-[9px] sm:text-[10px] md:text-xs font-mono text-white/30 uppercase tracking-widest italic">STREAMING</span>
                </div>
                  ...
                  <div className="slider-content flex gap-2 sm:gap-3 md:gap-4 lg:gap-6 w-max">
                    {displayScreenshots.map((src, i) => (
                      ...
                      >
                        ...
                        <div className="absolute bottom-1 sm:bottom-2 right-1 sm:right-2 text-[8px] sm:text-[9px] md:text-[10px] font-mono text-white/40 uppercase">CAM_0{(i % screenshots.length) + 1}</div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <aside className="lg:col-span-4 detail-sidebar space-y-8 sm:space-y-10 md:space-y-12 border-t lg:border-t-0 lg:border-l border-white/10 pt-8 sm:pt-10 lg:pt-0 lg:pl-8 md:lg:pl-12">
            <div className="space-y-4 sm:space-y-6">
              <h4 className="text-[11px] sm:text-xs md:text-sm font-mono tracking-[0.4em] sm:tracking-[0.5em] text-white/70 uppercase">ACTIONS</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3 sm:gap-4">
                {project.demoUrl && project.demoUrl !== '#' && (
                  <a href={project.demoUrl} target="_blank" rel="noreferrer" className="group flex items-center justify-between p-3 sm:p-4 md:p-6 min-h-[44px] border border-white/10 hover:bg-white hover:text-black transition-all text-[11px] sm:text-xs md:text-sm">
                ...
                <a href={project.repoUrl || "#"} target="_blank" rel="noreferrer" className="group flex items-center justify-between p-3 sm:p-4 md:p-6 min-h-[44px] border border-white/10 hover:bg-white/10 transition-all text-[11px] sm:text-xs md:text-sm">
            ...
            <div className="space-y-4 sm:space-y-6">
              <h4 className="text-[11px] sm:text-xs md:text-sm font-mono tracking-[0.4em] sm:tracking-[0.5em] text-white/70 uppercase">SPECS</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-x-6 sm:gap-x-8 gap-y-3 sm:gap-y-4">
                {[
                  ...
                  <div key={i} className="flex flex-col gap-1 border-b border-white/10 pb-3 sm:pb-4">
                    <span className="text-[10px] sm:text-[11px] md:text-xs font-mono text-white/50 uppercase tracking-widest">{spec.label}</span>
                    <span className="text-[11px] sm:text-xs md:text-sm font-mono text-white tracking-wider uppercase break-words">{spec.value}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4 sm:space-y-6">
              <h4 className="text-[11px] sm:text-xs md:text-sm font-mono tracking-[0.4em] sm:tracking-[0.5em] text-white/70 uppercase">MILESTONES</h4>
              <div className="space-y-3 sm:space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-center gap-2 sm:gap-4 group">
                    <span className="text-[10px] sm:text-xs md:text-sm font-mono text-white/50 group-hover:text-white transition-colors whitespace-nowrap">{milestone.phase}</span>
                    <div className="h-px flex-1 bg-white/10 group-hover:bg-white/30 transition-colors" />
                    <span className="text-[10px] sm:text-[11px] md:text-xs font-mono text-white tracking-wider uppercase whitespace-nowrap">{milestone.detail}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4 sm:space-y-6">
              <h4 className="text-[11px] sm:text-xs md:text-sm font-mono tracking-[0.4em] sm:tracking-[0.5em] text-white/70 uppercase">CAPABILITIES</h4>
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {["RESILIENT", "DYNAMIC", "SCALABLE"].map(cap => (
                  <span key={cap} className="px-2 sm:px-3 py-1 bg-white/10 border border-white/20 text-[10px] sm:text-[11px] md:text-xs font-mono text-white/80 tracking-widest uppercase">
                    {cap}
                  </span>
                ))}
              </div>
