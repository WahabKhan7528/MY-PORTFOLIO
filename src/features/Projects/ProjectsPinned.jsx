...
export default function ProjectsPinned({ onProjectSelect, isHero = false }) {
  ...
  return (
    <section
      id="projects"
      ref={containerRef}
      className={`relative w-full bg-black text-white h-auto lg:h-screen overflow-visible lg:overflow-hidden ${isHero ? 'hero-section' : 'full-section'}`}
    >
      {/* Section Badge */}
      ...
      <div className="relative w-full h-full flex flex-col lg:block gap-16 lg:gap-0">
        {projects.map((project, index) => (
          <div
            key={project.id}
            className="project-slide relative lg:absolute lg:inset-0 flex items-center justify-center p-6 sm:p-10 md:p-20 lg:p-24 bg-black"
